...
export const MockDataProvider = ({ children }) => {
  ...
  const [currentUser, setCurrentUser] = useState({
    ...
    role: "student", // 'student', 'mentor', 'faculty'
    ...
    portfolioUrl: "",
    coursework: "",
    badges: [], // Array of earned badges: [{ name, icon, earnedAt }]
    preferences: {
      ...
      publicProfile: true,
      acceptingMentees: true // For mentors
    }
  })
  ...
  // Actions
  const submitTeamFile = (teamId, fileName) => {
  ...
  const getReceivedRequests = (mentorId = null) => {
    ...
    const filtered = connectionRequests.filter(req => req.receiver_id === targetMentorId)
    
    console.log('📥 GET RECEIVED REQUESTS:')
    console.log('  ├─ Looking for Mentor ID:', targetMentorId)
  ...
  const getPendingRequestsForMentor = (mentorId = null) => {
    ...
    return connectionRequests.filter(req => 
      req.receiver_id === targetMentorId && req.status === 'pending'
    )
  }
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (request.receiver_id !== currentUser.id) {
          reject({ error: "Unauthorized: You can only update your own requests" })
          return
        }
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const newEvent = {
          ...
          rsvp_link: eventData.rsvp_link || `/events/${Date.now()}/rsvp`,
          registered: false,
          createdAt: new Date().toISOString()
        }
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        try {
          ...
          console.log('  ✅ Competition created with ID:', newCompetition.id)
          
          // 2. Generate judge invitations automatically
          const newInvitations = generateJudgeInvites(newCompetition, alumni)
          
          if (newInvitations.length > 0) {
            // 3. Send invitation emails (simulated)
