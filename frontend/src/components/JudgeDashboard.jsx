...
export default function JudgeDashboard() {
  const { teams: contextTeams, scoreTeam } = useMockData()
  ...
  useEffect(() => {
    ...
    // Convert teams to projects format for display
    const projectsFromTeams = contextTeams
      .filter(team => team.fileSubmitted && (team.files || team.fileName))
      .map((team, index) => ({
        id: team.id,
        teamName: team.name,
        projectTitle: `${team.name} Project`,
        submittedAt: team.submittedAt || new Date().toISOString(),
        files: team.files || (team.fileName ? [{ 
          name: team.fileName, 
          link: '#', 
          type: team.fileName.split('.').pop()?.toLowerCase() || 'file' 
        }] : [])
      }))
    
    setProjects(projectsFromTeams)
    setLoading(false)
    
    console.log('✅ Judge Dashboard loaded:', {
      teams: contextTeams.length,
      projects: projectsFromTeams.length,
      submissions: projectsFromTeams.filter(p => p.files.length > 0).length
    })
  }, [contextTeams])
  ...
  const handleSaveScore = (scores) => {
    // Use shared context function to update scores
