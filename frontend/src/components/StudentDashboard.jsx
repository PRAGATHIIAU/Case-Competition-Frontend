...
import { Home, Calendar, Trophy, Users, User, Bell, Upload, CheckCircle2, MessageSquare, Settings, Search, LogOut } from 'lucide-react'
...
import StudentCompetitions from './student/StudentCompetitions'
...
export default function StudentDashboard() {
  ...
  // Get toggleEventRSVP from context
  const { toggleEventRSVP } = useMockData();
  ...
  const handleRSVP = async (eventId) => {
    console.log('📋 StudentDashboard: handleRSVP called');
    ...
    try {
      ...
      console.error('❌ StudentDashboard handleRSVP error:', error);
      throw error;
    }
    setEvents(events.map(event => 
      event.id === eventId 
        ...
        <div className="w-full px-2 sm:px-3 lg:px-4">
          <div className="flex justify-between items-center h-12 sm:h-14 gap-1.5 sm:gap-2">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
              ...
              <button
                onClick={logout}
                className="p-1 sm:p-1.5 text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-100 flex-shrink-0 text-xs sm:text-sm font-medium whitespace-nowrap flex items-center gap-1"
                aria-label="Logout"
                title="Logout"
              >
