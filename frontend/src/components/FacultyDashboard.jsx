...
import { BarChart3, Users, TrendingUp, LogOut, Activity, Clock, Calendar, Trophy, Mail, BookOpen, UserCheck, Settings, Shield } from 'lucide-react'
...
import { hasPermission, PERMISSIONS, getAccessLevelDescription } from '../utils/permissions'
...
export default function FacultyDashboard() {
  ...
  const navItems = [
    ...
    // Sensitive pages - only for Faculty (always visible for faculty)
    { id: 'judge-comments', label: 'Judge Comments', icon: Mail },
    { id: 'competition-scores', label: 'Competition Scores', icon: Trophy },
    { id: 'leaderboard', label: 'Leaderboard', icon: TrendingUp },
    { id: 'judge-feedback', label: 'Judge Feedback', icon: Mail },
  ]
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="w-64 bg-white shadow-lg flex flex-col h-screen sticky top-0">
        <div className="p-6 border-b border-gray-200 flex-shrink-0">
          <h1 className="text-xl font-bold text-tamu-maroon">Faculty Portal</h1>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          {loading ? (
            ...
            >
              ...
              {/* Judge Comments Tab - Only for Faculty */}
              {activeTab === 'judge-comments' && (
                <JudgeCommentsView />
              )}

              {/* Competition Scores Tab - Only for Faculty */}
              {activeTab === 'competition-scores' && (
                <CompetitionScoresView />
              )}

              {/* Leaderboard Tab - Only for Faculty */}
              {activeTab === 'leaderboard' && (
                <Leaderboard teams={mockTeams} />
              )}

              {/* Judge Feedback Tab - Only for Faculty */}
              {activeTab === 'judge-feedback' && (
                <JudgeFeedbackView />
              )}
            </motion.div>
          )}
